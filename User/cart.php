    ...
    <div class="cartTable">
        <table id="t01">
            ...
            foreach ($fetch as $cartItem) {
                ...
                // print_r($fetch2);
                // echo "<br>";
                $total1 = $fetch2['price'] * $cartItem['quantity'];
                $totalcost = $totalcost + $total1;
                $total2 = $fetch2['calories'] * $cartItem['quantity'];
                $totalcalories = $totalcalories + $total2;
            ?>
                <tr>
                    <td><?= $s_no ?></td>
                    ...
                    <td><?= $fetch2['calories'] * $cartItem['quantity'] ?></td>
                    <td><?= $fetch2['price'] * $cartItem['quantity'] ?></td>
                    <td><?= $cartItem['orderedat'] ?></td>
                    <td>
                        <form action="editcart.php" method="post">
                            <input type="hidden" name="deletecalories" value="<?=($fetch2['calories'] * $cartItem['quantity'])?>">
            ...
            if ($totalcost == 0) {
                echo  '<td colspan="9"><h3 style="text-align:center;font-weight:bold;font-size:25px"><br>Cart is empty</h3></td>';
            }
            ?>
        ...
        if ($totalcost != 0) { ?>
            <div class="last">
                ...
                <form style="background-color: #0005007d;" action="payment.php" method="POST">
                    <!-- <input type="hidden" name="additionalcalories" value="<?= $totalcalories ?>"> -->
